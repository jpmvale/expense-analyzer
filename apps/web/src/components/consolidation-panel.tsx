import { AlertTriangleIcon, ChevronDownIcon, MergeIcon } from 'lucide-react';
  ...
  return (
    <li className={cn('px-4 py-3 sm:px-5', busy && 'opacity-50')}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {/*
         * A bloqueada não ganha botão. Não é proteção — o servidor aplica se
         * mandarem —, é recusa de fingir que as duas são a mesma decisão: uma
         * arruma regra, a outra desfaz classificação que alguém fez de propósito.
         */}
        {!blocked && (
          <Button
            size="sm"
...
export function ConsolidationPanel({
  ...
  const safe = suggestions.filter((s) => s.conflicts.length === 0).length;
    ...
    <Card className="mb-4 overflow-hidden">
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          ...
          também o sufixo novo que ainda vai chegar. {safe} de {suggestions.length} sem conflito.
        </CardDescription>
      </CardHeader>
